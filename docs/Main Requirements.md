...
**\*\*Компоненты:\*\***  
1\.  **\*\*Header:\*\*** Логотип и кнопка "Мои черновики".  
...
| content | TEXT | Полный текст черновика (с Unicode символами) |
| created\_at | DATETIME | Дата создания |
| updated\_at | DATETIME | Дата последнего изменения |
| preview\_snippet | TEXT | Первые 50 символов для заголовка в списке |

### **5.2. API Endpoints**

Необходимо реализовать простой REST API:

1. GET /api/drafts — получить список всех черновиков (id, date, snippet).  
2. POST /api/drafts — сохранить новый черновик (или обновить, если передан id).  
   * *Body:* { "content": "..." }  
3. GET /api/drafts/:id — получить полный текст черновика.  
4. DELETE /api/drafts/:id — удалить черновик.
